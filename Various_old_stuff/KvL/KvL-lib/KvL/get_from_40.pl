...
my $reg_conf40 = "/usr/bin/kvl_lib/ensembl_init_46";

open (TO_40, "$tempdir/temp/to_46.txt");
my @input = <TO_40>;
...
Bio::EnsEMBL::Registry->load_all($reg_conf40);
#print "gaat het hier al mis?\n";
my $slice_adaptor40 = Bio::EnsEMBL::Registry->get_adaptor($species,'core','Slice');
my $transcript_adaptor40 = Bio::EnsEMBL::Registry->get_adaptor($species,'core','Transcript');
my $transcript = $transcript_adaptor40->fetch_by_stable_id($transcript_id);
#print "of misschien hier?\n$chromosome\n";


my $slice40 = $slice_adaptor40->fetch_by_region('chromosome', $chromosome, ($mirna_start-100), ($mirna_end+100));
my $sequence_large = $slice40 ->seq();

# print "hier in elk geval?\n";

$slice40 = $slice_adaptor40->fetch_by_region('chromosome', $chromosome, $mirna_start, $mirna_end);
my $sequence_small = $slice40 ->seq();
