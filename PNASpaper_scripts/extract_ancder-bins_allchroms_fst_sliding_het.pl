...
while (<>){
	...
	if (scalar @inbin < $binsize){
		...
		foreach my $element (@inbin){
			...
			++$countall;
			$hetfreqgr1 = $group1freq;
			if ($hetfreqgr1>0.5){
				$hetfreqgr1 = 1-$hetfreqgr1;
			}
			$tothetfreqgr1=$tothetfreqgr1+$hetfreqgr1;	
			if ($ancstate =~ /[ACGT]/){
                		++$count;
		...
		$tothetfreqfreqgr1=$tothetfreqgr1/$countall;
                print "$chrom\t$bin\t$group1avgfreq\t$group2avgfreq\t$ratio\t$count\t$fstavg\t$allpos\t$tothetfreqfreqgr1\n";
		shift(@inbin);
	}
}
