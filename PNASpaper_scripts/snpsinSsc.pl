...
while (<FILE>){
	...
	if ($mislukt1/$totaal1 < 0.5){
		...
		if (@alleles1 && ($ref1count+$nonref1count)>0){
			
			#print "$line\n";
			my $reffreq1=$ref1count/($ref1count+$nonref1count);
			if (scalar @alleles1 >1){
				++$snpsgroup1;	
				print "$chrom\t$pos\t$pos\t$oldref\t$oldnonref\tnumlines:$numlines\t$flag snpgroup1:$snpsgroup1 group1:$reffreq1 $geluktgroup1:$geluktgroup2:$geluktgroup3\n"
			}
