...
sub fetch_snps {
	...
	my $query = "select SNP from snps_build10_2 inner join hapmap2_call_rate using (SNP) where Chromosome = 6 and Position < 52336598 and Position > 48585961 and Alt_Position = 0 and Call_Freq >0.95 order by Position";
	my $sql = $pig_hapmap->prepare($query);	
	$sql->execute();
	my @target_snps;
	while (my $row = $sql->fetchrow_arrayref) {
