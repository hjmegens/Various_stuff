...
sub fetch_snps {
	...
	#my $query = "select SNP from snps_build10 inner join hapmap2_call_rate using (SNP) where Chromosome = 'X' and Alt_Position = 0 and Position < 60000000 and Position > 54000000 and Call_Freq > 0.90 order by Position";
	my $query = "select SNP from snps_build10 inner join hapmap2_call_rate using (SNP) where Chromosome = 'Y' and Call_Freq > 0.2";
	my $sql = $pig_hapmap->prepare($query);	
	$sql->execute();
	my @target_snps;
	while (my $row = $sql->fetchrow_arrayref) {
...
sub make_info_file {
	...
	my @snps = @$snpref;
	foreach my $snp (@snps) {
		my $query = "select Position from snps_build10 where SNP = ('$snp')";
		my $sql = $pig_hapmap->prepare($query);	
		$sql->execute();
		my @target_snps;
		while (my $row = $sql->fetchrow_arrayref) {
