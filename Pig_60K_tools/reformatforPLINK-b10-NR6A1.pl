...
sub fetch_snps {
	...
	my $query = "select SNP from snps_build10_2 inner join hapmap2_call_rate using (SNP) where Chromosome = '1' and Alt_Position = 0 and Call_Freq > 0.90  and Position < 299500000 and Position > 299000000 order by Position";
	my $sql = $pig_hapmap->prepare($query);	
	$sql->execute();
	my @target_snps;
	while (my $row = $sql->fetchrow_arrayref) {
