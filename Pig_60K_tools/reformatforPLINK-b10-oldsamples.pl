...
sub fetch_snps {
	...
	my $query = "select SNP from snps_build10 inner join hapmap2_call_rate using (SNP) where Chromosome = $target and Alt_Position = 0 and Call_Freq >0.95 order by Position";
	my $sql = $pig_hapmap->prepare($query);	
	$sql->execute();
	my @target_snps;
	while (my $row = $sql->fetchrow_arrayref) {
